...
│   └── kotlin/com/aries/tutorial2/adapter/  # Kotlin アダプター例
│       ├── EventAdapter.kt
│       ├── LoginAdapter.kt
│       ├── SSOLoginAdapter.kt
...
#### アダプター ID の設定方法
1.  **JENNIFER 管理コンソール**にアクセス: `設定` > `拡張モジュール` メニューに移動します。
...
### 1. 固有のパッケージ名を使用
JENNIFER サーバーの内部クラスとの衝突を防ぐために、サーバーが内部で使用するパッケージ名 (`com.aries.*`, `aries.*`) は使用しないでください。
...
### 4. ライブラリの重複に注意
`provided` スコープで設定されたライブラリ以外の外部ライブラ리を追加する場合、そのライブラリが既にサーバーに存在するか確認してください。バージョンの異なる同一ライブラリが重複すると、クラスの衝突が発生する可能性があります。
...
すべてのアダプターは Kotlin でも実装されています。`com.aries.tutorial2.adapter` パッケージを参照してください。
