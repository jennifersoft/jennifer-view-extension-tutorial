...
#### アダプター ID の設定方法
1.  **JENNIFER 管理コンソール**にアクセス: `設定` > `SMTPおよびアダプター (+DB Plan)` メニューに移動します。
2.  **アダプターの設定画面へ**: アダプターテーブルの右側にあるボタンをクリックし、表示されるメニューから **[オプション]** を選択します。
3.  **アダプター ID の入力**: 設定ポップアップ画面で、**[アダプター ID]** フィールドに値を 입력합니다 (例: `event_adapter`)。
4.  **プロパティの登録**: 同じポップアップ画面の [カスタムプロパティ] 領域に Key (例: `subject`) と Value を入力して保存します。
5.  ソースコードで上記で設定した **[アダプター ID]** を第 1 パラメータとして渡して値を読み取ります。
